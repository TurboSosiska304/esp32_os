...
/* ── compact 3×5 pixel font helpers ────────────────────────── */
...
static uint8_t display_compact_glyph(char character, int row)
{
    ...
    if (character >= 'A' && character <= 'F') return hex_letters[character - 'A'][row];
    switch (character) {
    case 'I': return labels[0][row]; case 'P': return labels[1][row];
    case 'C': return labels[2][row]; case 'U': return labels[3][row];
    case 'M': return labels[4][row]; case 'V': return labels[5][row];
    case 'S': return labels[6][row]; case 'H': return labels[7][row];
        ...
        for (int row = 0; row < 5; ++row) {
            ...
            for (int column = 0; column < 3; ++column) {
                if ((bitmap & (1U << (2 - column))) != 0U) {
                    display_rectangle(x + (int)(offset * 4U * scale) + column * scale,
                                      y + row * scale, scale, scale, color);
                }
            }
        }
...
static void display_draw_dashboard(uint32_t sequence, uint64_t uptime_ms)
{
    ...
    display_compact_text(14, 60, "CPU:", 3, DISPLAY_ACCENT);
    display_compact_text(74, 60, cpu_text, 3, DISPLAY_WARNING);
    display_compact_text(128, 60, "MHZ", 3, DISPLAY_WARNING);
    display_rectangle(14, 90, 252, 1, DISPLAY_DIM);
    display_compact_text(14, 104, "HB:", 3, DISPLAY_ACCENT);
    display_number(84, 98, sequence, 5, DISPLAY_FOREGROUND);
    display_compact_text(14, 150, "UP:", 3, DISPLAY_ACCENT);
    display_number(84, 144, (uint32_t)(uptime_ms / 1000U), 5, DISPLAY_WARNING);
    display_compact_text(220, 150, "S", 3, DISPLAY_WARNING);
...
static void display_task(void *argument)
{
    ...
    for (;;) {
        ...
        if (micro_os_queue_receive(events, &event, MICRO_OS_WAIT_FOREVER) == MICRO_OS_OK) {
            ...
            display_event_t latest = event;
            while (micro_os_queue_receive(events, &latest, 0) == MICRO_OS_OK) {
                event = latest;
            }
